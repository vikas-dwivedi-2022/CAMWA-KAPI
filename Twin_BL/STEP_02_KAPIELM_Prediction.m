clc; clear; close all;
rng(42);
%% TWIN BOUNDARY LAYER CASE
eta=0.1; n_unif=1500;
% nu=0.0001; 
% w_f   = [0.49525 , 0.49573];  
% w_mu  = [0.0011201 , 0.99742];   
% w_tau = [0.084556, 0.079276 ];     
% w_lam = [0.73396,  0.71691];   

% nu=0.001; 
% w_f   = [0.50011 , 0.50204];  
% w_mu  = [0.0017115 , 0.99723];   
% w_tau = [0.087752, 0.084981 ];     
% w_lam = [0.61526,  0.71539];   

nu=0.01; 
w_f   = [0.5047 , 0.5012];  
w_mu  = [0.0030567 , 0.99542];   
w_tau = [0.099949, 0.091646 ];     
w_lam = [0.99286,  0.97807];     
...
function J_val = KAPI_net(nu, eta, n_unif, w_f, w_mu, w_tau, w_lam)
    ...
    X_pde_test=[linspace(0, 0.1, n_unif)';linspace(0.1, 0.9, 4*n_unif)';linspace(0.9, 1, n_unif)'];
...
function output_cell = Data_and_Bases_1D(fixed_params, tunable_params)
    ...
    %% Local Part (Adaptive)
    eta_tau = eta * w_tau/3;
    n_adap = round(n_unif .* w_f ./ sum(w_f)); % MAXIMUM 100%!
    ...
    scatter_centers_vs_widths(alpha_star, sig, 'FIG_RBF_1D_centers.png');
...
function [H, b] = CONSTRUCT_H_AND_b(X_pde, X_bc, m, alpha, nu)
    ...
    b = [zeros(Nc, 1); 1; 1];  % PDE RHS = 0, BCs: u(0) = 0, u(1) = 1
end
...
%     Saves a 300 DPI PNG plot with solution and error overlay
    ...
    plot(X_test, u_kapi, 'r--', 'LineWidth', 2); xlim([-0.05,1.05]);
    ylabel_str = ['$u(x;\, \nu=' sprintf('%.2f', nu) ')$'];%%%%%%%%%%%%%%%%%%%%%%%
    ...
    legend({'True', 'Predicted', 'Abs. Error'}, 'Location', 'north', ...
